...
class ReleaseHygiene(unittest.TestCase):
    ...
    def test_repository_infrastructure_is_not_part_of_the_bundle(self) -> None:
        ...
        try:
            ...
            fx.write(copy, "dist/pacote-antigo.zip", "zip de um build anterior dentro do repositório")
            built = run(copy / "tools" / "build_release.py", "--out-dir", str(copy / "dist"), "--name", "pacote-v9.9.9")
            self.assertEqual(built.returncode, 0, built.stderr)
            with zipfile.ZipFile(copy / "dist" / "pacote-v9.9.9.zip") as archive:
                names = archive.namelist()
            self.assertTrue(all(n.startswith("pacote-v9.9.9/") for n in names))
            self.assertFalse(any("/.github/" in n or n.endswith("/.gitattributes") or "/dist/" in n for n in names))
            self.assertNotIn(".github", (copy / "SHA256SUMS.txt").read_text(encoding="utf-8"))
            verified = run(copy / "tools" / "build_release.py", "--verify-zip", str(copy / "dist" / "pacote-v9.9.9.zip"))
            self.assertEqual(verified.returncode, 0, verified.stderr)
